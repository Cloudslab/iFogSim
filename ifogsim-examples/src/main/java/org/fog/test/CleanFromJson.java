...
public class CleanFromJson {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 */
			URL topologyResource = CleanFromJson.class.getClassLoader().getResource("topologies/routerTopology");
			if (topologyResource == null) {
				throw new IllegalStateException("topologies/routerTopology not found on classpath");
			}
			PhysicalTopology physicalTopology = JsonToTopology.getPhysicalTopology(broker.getId(), appId,
					Paths.get(topologyResource.toURI()).toString());
